		...
		<div id="lgw-footer">
			<p>
				...
				| <a href="<?php echo admin_url( 'admin.php?page=lengow&tab=lengow_admin_toolbox' ); ?>"
					class="sub-link"
					title="<?php echo esc_attr( $locale->t( 'footer.toolbox' ) ); ?>">
					<?php echo esc_html( $locale->t( 'footer.toolbox' ) ); ?>
				</a>
				| <a href="<?php echo admin_url( 'admin.php?page=lengow&tab=lengow_admin_legals' ); ?>"
					class="sub-link"
					title="<?php echo esc_attr( $locale->t( 'footer.legals' ) ); ?>">
				...
				<div class="lgw-modalbox-body">
					<div class="lgw-row flexbox-vertical-center">
						...
						<div class="lgw-col-7">
							...
							<p>
								<?php echo esc_html( $locale->t( 'update.start_now' ) ); ?>
								<a href="<?php echo esc_url( $plugin_links[ Lengow_Sync::LINK_TYPE_CHANGELOG ] ); ?>"
									target="_blank">
									<?php echo esc_html( $locale->t( 'update.link_changelog' ) ); ?>
								</a>
							</p>
							<div class="lgw-content-section mod-small">
								<h2><?php echo esc_html( $locale->t( 'update.step_one' ) ); ?></h2>
								<p class="no-margin-bottom">
									<?php echo esc_html( $locale->t( 'update.download_last_version' ) ); ?>
								</p>
							...
							<div class="lgw-content-section mod-small">
								<h2><?php echo esc_html( $locale->t( 'update.step_two' ) ); ?></h2>
								<p class="no-margin-bottom">
									<a href="<?php echo esc_url( $plugin_links[ Lengow_Sync::LINK_TYPE_UPDATE_GUIDE ] ); ?>"
										target="_blank"><?php echo esc_html( $locale->t( 'update.link_follow' ) ); ?></a>
									<?php echo esc_html( $locale->t( 'update.update_procedure' ) ); ?>
								</p>
								<p class="text-lesser text-italic">
									<?php echo esc_html( $locale->t( 'update.not_working' ) ); ?>
									<a href="<?php echo esc_url( $plugin_links[ Lengow_Sync::LINK_TYPE_SUPPORT ] ); ?>"
										target="_blank"><?php echo esc_html( $locale->t( 'update.customer_success_team' ) ); ?></a>
								</p>
							</div>
							<div class="flexbox-vertical-center margin-standard">
								...
								<?php if ( $show_plugin_upgrade_modal ) : ?>
									<button class="btn-link sub-link no-margin-top text-small js-upgrade-plugin-modal-remind-me">
										<?php echo esc_html( $locale->t( 'update.button_remind_me_later' ) ); ?>
									</button>
								<?php endif; ?>
							</div>
						</div>
