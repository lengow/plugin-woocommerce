...
<ul class="nav nav-pills lengow-nav lengow-nav-top">
    <li role="presentation" id="lengow_logo">
        <a href="<?php echo esc_url( admin_url( 'admin.php?page=lengow&tab=lengow_admin_dashboard' ) ); ?>">
            <img src="/wp-content/plugins/lengow/assets/images/lengow-white.png" alt="lengow">
        </a>
    </li>
    <li role="presentation"
        class="<?php echo ( isset( $current_page ) && 'lengow_admin_products' === $current_page ) ? 'active' : ''; ?>">
        <a href="
			<?php echo esc_url( admin_url( 'admin.php?page=lengow&tab=lengow_admin_products' ) ); ?>">
			<?php echo esc_html( $locale->t( 'menu.product' ) ); ?>
        </a>
    </li>
    <li role="presentation"
        class="<?php echo esc_attr( ( isset( $current_page ) && 'lengow_admin_orders' === $current_page ) ? 'active' : '' ); ?>"
        id="js-menugotoimport">
        <a href="
			<?php echo esc_url( admin_url( 'admin.php?page=lengow&tab=lengow_admin_orders' ) ); ?>">
			<?php echo esc_html( $locale->t( 'menu.order' ) ); ?>
			<?php if ( $total_pending_order > 0 ) : ?>
                <span class="lengow-nav-notif"><?php echo esc_html( $total_pending_order ); ?></span>
			<?php endif; ?>
        </a>
    </li>
    <li class="lengow_float_right <?php echo esc_attr( ( isset( $current_page ) && 'lengow_admin_settings' === $current_page ) ? 'active' : '' ); ?>"
        id="menugotosetting">
        <a href="<?php echo esc_url( admin_url( 'admin.php?page=lengow&tab=lengow_admin_settings' ) ); ?>"
           class="lengow_link_tooltip"
           data-placement="bottom" data-original-title="<?php echo esc_attr( $locale->t( 'menu.global_parameter' ) ); ?>">
            <i class="fa fa-cog fa-2x"></i>
        </a>
    </li>
    <li class="lengow_float_right <?php echo esc_attr( ( isset( $current_page ) && 'lengow_admin_help' === $current_page ) ? 'active' : '' ); ?>"
        id="menugotohelp">
        <a href="<?php echo esc_url( admin_url( 'admin.php?page=lengow&tab=lengow_admin_help' ) ); ?>"
           class="lengow_link_tooltip"
           data-placement="bottom" data-original-title="<?php echo esc_attr( $locale->t( 'menu.help' ) ); ?>">
