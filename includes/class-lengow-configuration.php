...
class Lengow_Configuration {
	...
	const PLUGIN_ENV                           = 'lengow_plugin_env';
	...
	const RETURN_TRACKING_NUMBER               = 'lengow_return_tracking_number';
	const RETURN_CARRIER                       = 'lengow_return_carrier';
	...
	public static $generic_param_keys = array(
		...
		self::RETURN_CARRIER                       => 'lengow_return_carrier',
		self::RETURN_TRACKING_NUMBER               => 'lengow_return_tracking_number'
	);
		...
		if ( null === $keys ) {
			...
			$keys   = array(
				...
				self::EXPORT_PRODUCT_TYPES                 => array(
					...
					self::PARAM_DEFAULT_VALUE => self::PRODUCT_TYPES,
					self::PARAM_RETURN        => self::RETURN_TYPE_ARRAY,
				),
				...
				self::RETURN_TRACKING_NUMBER               => array(
					self::PARAM_EXPORT_TOOLBOX => false,
					self::PARAM_LABEL          => $locale->t( 'lengow_settings.lengow_return_tracking_title' ),
					self::PARAM_RETURN         => self::RETURN_TYPE_BOOLEAN,
				),
				self::RETURN_CARRIER                       => array(
					self::PARAM_EXPORT_TOOLBOX => false,
					self::PARAM_LABEL          => $locale->t( 'lengow_settings.lengow_return_carrier_title' ),
					self::PARAM_RETURN         => self::RETURN_TYPE_BOOLEAN,
				),
			);
		}
	...
	public static function get_access_id() {
		if ( '' !== self::get( self::ACCOUNT_ID ) && '' !== self::get( self::ACCESS_TOKEN ) && '' !== self::get( self::SECRET )
		) {
			return array(
	...
	public static function get_all_values( $all = true, $shop = false, $toolbox = false ) {
		...
		foreach ( $keys as $key => $key_params ) {
			if ( $all ) {
				...
				$value = null;
				if ( ( isset( $key_params[ self::PARAM_EXPORT ] ) && ! $key_params[ self::PARAM_EXPORT ] ) || ( $toolbox && isset( $key_params[ self::PARAM_EXPORT_TOOLBOX ] ) && ! $key_params[ self::PARAM_EXPORT_TOOLBOX ]
						)
				) {
					continue;
				}
				if ( $shop ) {
					if ( isset( $key_params[ self::PARAM_SHOP ] ) && $key_params[ self::PARAM_SHOP ] ) {
						...
						$generic_key          = self::CMS_TOKEN === $key ? self::$generic_param_keys[ self::SHOP_TOKEN ] : self::$generic_param_keys[ $key ];
						$rows[ $generic_key ] = self::get_value_with_correct_type( $key, $value );
					}
				} elseif ( isset( $key_params[ self::PARAM_GLOBAL ] ) && $key_params[ self::PARAM_GLOBAL ] ) {
					$value                                     = self::get( $key );
	 ...
	 * @return string
	 */
	public static function get_plugin_environment() {
		$env = self::get( self::PLUGIN_ENV );

		if ( ! $env ) {
			$keyParams = self::get_keys( self::PLUGIN_ENV );
		...
		if ( isset( $key_params[ self::PARAM_RETURN ] ) ) {
			switch ( $key_params[ self::PARAM_RETURN ] ) {
				...
				case self::RETURN_TYPE_ARRAY:
					if ( ! is_array( $value ) ) {
						return ! empty( $value ) ? explode( ';', trim( str_replace( array( "\r\n", ',', ' ' ), ';', $value ), ';' ) ) : array();
					}
			}
		}
