...
class Lengow_Toolbox_Element {
	...
	public function get_global_information() {
		...
		$checklist   = array(
			array(
				self::DATA_TITLE   => $this->locale->t( 'toolbox.index.wordpress_version' ),
				self::DATA_MESSAGE => $plugin_data[ Lengow_Toolbox::PLUGIN_CMS_VERSION ],
			),
			array(
				self::DATA_TITLE   => $this->locale->t( 'toolbox.index.woocommerce_version' ),
			...
			array(
				self::DATA_TITLE => $this->locale->t( 'toolbox.index.ip_enabled' ),
	...
	public function get_import_information() {
		...
		if ( 0 === $last_synchronization ) {
			$last_import_date = $this->locale->t( 'toolbox.index.last_import_none' );
	...
	public function get_export_information() {
		...
		$checklist             = array(
			...
			array(
				self::DATA_TITLE   => $this->locale->t( 'toolbox.index.export_product_types' ),
	...
	public function check_file_md5() {
		...
		$checksum_title = $this->locale->t( 'toolbox.checksum.summary' );
